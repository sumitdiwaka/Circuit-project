...
function Register() {
    ...
    const onSubmit = async (e) => { e.preventDefault(); if (password !== password2) { toast.error('Passwords do not match'); } else { try { await authService.register({ username, email, password }); navigate('/'); toast.success('Welcome!'); } catch (error) { const message = (error.response?.data?.message) || error.message || error.toString(); toast.error(message); } } };
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="min-h-[calc(100vh-120px)] flex items-center justify-center">
            <div className="w-full max-w-md bg-black/20 backdrop-blur-md rounded-2xl p-8 border border-white/10 shadow-2xl">
                ...
                <p className="text-center text-gray-400 mb-8">Start your journey with us.</p>
                <form onSubmit={onSubmit} className="space-y-6">
                    <div className="relative">
                        <FaUser className="absolute top-1/2 left-4 -translate-y-1/2 text-gray-400" />
                        <input type="text" name="username" value={username} onChange={onChange} placeholder="Username" required className="w-full pl-12 pr-4 py-3 bg-secondary-dark text-white border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent-pink" />
                    </div>
                    <div className="relative">
                    ...
                    <div className="relative">
                        <FaLock className="absolute top-1/2 left-4 -translate-y-1/2 text-gray-400" />
                        <input type="password" name="password2" value={password2} onChange={onChange} placeholder="Confirm Password" required className="w-full pl-12 pr-4 py-3 bg-secondary-dark text-white border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent-pink" />
                    </div>
                ...
                <p className="text-center text-gray-400 mt-8">
                    Already have an account? <Link to="/login" className="font-semibold text-accent-pink hover:underline">Log in</Link>
                </p>
            </div>
        </motion.div>
    );
